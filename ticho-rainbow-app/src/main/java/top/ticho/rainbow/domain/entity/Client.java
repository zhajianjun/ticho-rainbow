...
public class Client implements Entity {
    ...
    public void enable() {
        ...
        TiAssert.isTrue(Objects.equals(this.status, disable.code()),
            TiStrUtil.format("只有[{}]状态才能执行启用操作，客户端：{}", disable.message(), name));
        boolean isExired = Objects.nonNull(expireAt) && expireAt.isBefore(LocalDateTime.now());
        TiAssert.isTrue(!isExired, TiStrUtil.format("客户端[{}]已过期", name));
    ...
    public void disable() {
        ...
        TiAssert.isTrue(Objects.equals(this.status, enable.code()),
            TiStrUtil.format("只有[{}]状态才能执行禁用操作，客户端：{}", enable.message(), name));
