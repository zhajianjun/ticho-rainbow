...
public class Task implements Entity {
    ...
    public void modify(TaskModifyVO taskModifyVo) {
        if (isEnable()) {
            TiAssert.isTrue(Objects.equals(content, taskModifyVo.content()), TiStrUtil.format("任务已启用，任务内容不能修改，任务：{}", name));
            TiAssert.isTrue(Objects.equals(param, taskModifyVo.param()), TiStrUtil.format("任务已启用，任务参数不能修改，任务：{}", name));
            TiAssert.isTrue(Objects.equals(cronExpression, taskModifyVo.cronExpression()), TiStrUtil.format("任务已启用，Cron执行表达式不能修改，任务：{}", name));
        }
    ...
    public void enable() {
        ...
        TiAssert.isTrue(Objects.equals(this.status, disable.code()),
            TiStrUtil.format("只有[{}]状态才能执行启用操作，任务：{}", disable.message(), name));
    ...
    public void disable() {
        ...
        TiAssert.isTrue(Objects.equals(this.status, enable.code()),
            TiStrUtil.format("只有[{}]状态才能执行禁用操作，任务：{}", enable.message(), name));
