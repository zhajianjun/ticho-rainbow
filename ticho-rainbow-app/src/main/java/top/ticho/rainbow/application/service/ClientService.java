...
public class ClientService {
    ...
    public void exportExcel(ClientQuery query) throws IOException {
        ...
        String fileName = "客户端信息导出-" + LocalDateTime.now().format(DateTimeFormatter.ofPattern(DateConst.PURE_DATETIME_PATTERN));
    ...
    private boolean modifyBatch(List<VersionModifyCommand> modifys, Consumer<Client> modifyHandle, Consumer<Client> modifyToDbAfterHandle) {
        ...
        Map<Long, Client> clientMap = clients.stream().collect(Collectors.toMap(Client::getId, Function.identity(), (o, n) -> o));
        for (VersionModifyCommand modify : modifys) {
            Client client = clientMap.get(modify.getId());
            TiAssert.isNotNull(client, TiStrUtil.format("操作失败, 数据不存在, id: {}", modify.getId()));
            client.checkVersion(modify.getVersion(), TiStrUtil.format("数据已被修改，请刷新后重试, 客户端: {}", client.getName()));
            // 修改逻辑
