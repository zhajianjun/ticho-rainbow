...
public class AppListenHandler extends SimpleChannelInboundHandler<ByteBuf> {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        // 如果客户端连接异常，则关闭请求通道
        if (Objects.isNull(clientInfo) || Objects.isNull(clientInfo.getChannel()) || !TichoUtil.isActive(clientInfo.getChannel())) {
        ...
        if (MapUtil.isNotEmpty(requestChannels) && requestChannels.size() >= maxRequests) {
            ...
            TichoUtil.close(oldRequestChannel);
        }
    ...
    protected void channelRead0(ChannelHandlerContext ctx, ByteBuf buf) {
        ...
        if (!TichoUtil.isActive(clientChannel)) {
            requestChannel.close();
            return;
        }
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        ...
        TichoUtil.close(clientChannel.attr(CommConst.CHANNEL).get());
